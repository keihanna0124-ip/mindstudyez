...
const Header: React.FC<HeaderProps> = ({ activeSection, setActiveSection, isDarkMode, toggleDarkMode, onReset }) => {
  const navItems = [
    ...
    <header className="sticky top-0 z-50 glass transition-all duration-300 border-b border-white/20 dark:border-slate-800/50">
      <div className="container mx-auto px-6 py-4 flex items-center justify-between gap-4">
        ...
        <nav className="hidden md:flex items-center gap-2 bg-slate-100/30 dark:bg-slate-900/40 p-1.5 rounded-full backdrop-blur-3xl border border-white/10 shadow-inner">
          ...
          <button
            onClick={onReset}
            className="w-9 h-9 rounded-full flex items-center justify-center hover:bg-red-50 dark:hover:bg-red-900/20 transition-all text-slate-400 hover:text-red-500"
            title="Reset dữ liệu"
          >
